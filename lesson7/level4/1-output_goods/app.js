...
let blockOfProducts = document.querySelector('div.products');
...
function clickHandler(event) {
    ...
    let typeCategory = event.target.dataset.type;
...
function showCategory(category) {
    ...
    currentProducts.forEach(product => {
        blockOfProducts.insertAdjacentHTML('beforeend', getProductMarkup(product));
    });
}
...
function getProductMarkup(product) {
    ...
    text += `<div>${product.name}</div>`;
    text += `<img src="${product.imageUrl}" alt="">`;
    text += `<div>${product.price}</div>`;
    text += `<a href="https://example.com/producs/${product.id}">Подробнее</a>`
    text += '</div>';
    return text;
}
