...
function hideMoreText(card) {
    ...
    card.removeChild(card.querySelector(".desc"));
...
function showMoreText(card) {
    ...
    let text = 'Known for her sculptural takes on traditional tailoring, Australian arbiter of cool Kym Ellery teams up with Moda Operandi.';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    let newBlock = document.createElement('div');
    newBlock.className = 'desc';
    newBlock.innerText = text;
    card.productName.insertAdjacentElement('afterEnd', newBlock);
